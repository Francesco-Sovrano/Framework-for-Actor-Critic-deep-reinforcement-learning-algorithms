...
class StateSituationGenerator(ABC):
	def __init__(self):
		self._set_shape()
	...
	def compute_state(self, history):
...
class CroppedView_StateGenerator(StateSituationGenerator): # 6 situations
...
class CroppedView_1b_3L_StateGenerator(CroppedView_StateGenerator): # 4 situations
	...
	def build_state(self, info):
		state = self.empty_state()
		state = self.set_channel(1, self.player_position, state, info.get_list_of_positions_by_tile("%"), 4) # stairs
		state = self.set_channel(2, self.player_position, state, info.get_list_of_positions_by_tile("|"), 8) # walls
		state = self.set_channel(2, self.player_position, state, info.get_list_of_positions_by_tile("-"), 8) # walls
		state = self.set_channel(0, self.player_position, state, info.get_list_of_positions_by_tile("+"), 16) # doors
		state = self.set_channel(0, self.player_position, state, info.get_list_of_positions_by_tile("#"), 16) # tunnel
