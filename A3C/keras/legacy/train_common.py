...
MultiAgentModel = Vars['MultiAgentModel']
...
class Application(object):
    ...
    def train_function(self, parallel_index, preparing):
        ...
        while True:
            ...
            diff_global_t = trainer.process(self.sess, self.global_t, self.summary_writer, self.summary_op,
                                            self.score_input)
    ...
    def build_global_network(self, learning_rate_input):
        ...
        self.global_network = MultiAgentModel(-1, state_shape, agents_count, action_size, flags.entropy_beta,
                                              self.device)
        return RMSPropApplier(learning_rate=learning_rate_input, decay=flags.rmsp_alpha, momentum=0.0,
                              epsilon=flags.rmsp_epsilon, clip_norm=flags.grad_norm_clip, device=self.device)

    def build_local_networks(self, learning_rate_input, grad_applier):
        initial_learning_rate = log_uniform(flags.initial_alpha_low, flags.initial_alpha_high,
                                            flags.initial_alpha_log_rate)
        self.trainers = []
        for i in range(flags.parallel_size):
            trainer = Trainer(i, self.global_network, initial_learning_rate, learning_rate_input, grad_applier,
                              flags.env_type, flags.entropy_beta, flags.local_t_max, flags.gamma, flags.max_time_step,
                              self.device)
