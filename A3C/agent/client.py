...
class Worker(object):
	...
	def run_batch(self, global_step):
		...
		while step < flags.max_batch_size and not self.terminal:
			...
			new_state, value, action, reward, self.terminal, cross_entropy, entropy = self.local_network.act( 
			...
			if flags.show_best_episodes or flags.show_all_episodes:
				self.frame_info_list.append( self.environment.get_frame_info(network=self.local_network, observation=self.environment.get_screen(), value=value, action=action, reward=reward, cross_entropy=cross_entropy, entropy=entropy) )
