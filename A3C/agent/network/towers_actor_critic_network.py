	...
	def _cnn_layer(self, input, scope, name="", share_trainables=True):
		...
		with tf.variable_scope(scope), tf.variable_scope("CNN{}".format(name), reuse=tf.AUTO_REUSE) as variable_scope:
			...
			with tf.variable_scope("tower_2"):
				tower2 = tf.layers.max_pooling2d(input, pool_size=(2, 2), strides=(2, 2), padding='SAME')
				...
				tower2 = tf.layers.max_pooling2d(tower2, pool_size=(max(1,input_shape[1]//2), max(1,input_shape[2]//2)), strides=(max(1,input_shape[1]//2), max(1,input_shape[2]//2)))
			...
			with tf.variable_scope("tower_3"):
				...
				tower3 = tf.layers.max_pooling2d(tower3, pool_size=(max(1,input_shape[1]//4), max(1,input_shape[2]//4)), strides=(max(1,input_shape[1]//4), max(1,input_shape[2]//4)))
