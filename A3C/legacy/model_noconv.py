...
from .lib import Conv2D
from .lib import MaxPool
...
class MultiAgentModel(object):
    def __init__(self, id, state_shape, agents_count, action_size, entropy_beta, device):
        ...
        for i in range(self.agent_count):
            self._agent_list.append(
                A3CModel(str(id) + "_" + str(i), self._agent_state_shape, action_size, entropy_beta, device))
...
class A3CModel(object):
    ...
    def __init__(self, id, state_shape, action_size, entropy_beta, device):
        ...
        self._value_conv1 = Conv2D(name="value_conv1", input=self._value_layer_shape, kernel=(3, 3), stride=(1, 1),
                                   depth=16)
        ...
        self._policy_conv1 = Conv2D(name="policy_conv1", input=self._policy_layer_shape, kernel=(3, 3), stride=(1, 1),
                                    depth=16)
    ...
    def _create_base_network(self):
        ...
        self.base_lstm_outputs, self.base_lstm_state = self._base_lstm_layer(conv_result, conv_size,
    ...
    def _base_lstm_layer(self, conv_result, conv_size, last_action_reward_input, initial_state_input, reuse=False):
        with tf.variable_scope("base_lstm{0}".format(self._id), reuse=reuse) as scope:
            ...
            conv_output_fc = tf.nn.relu(tf.matmul(conv_output_flat, weights_fc1) + biases_fc1, name='tower_out')
            ...
            lstm_input_reshaped = tf.reshape(lstm_input, [1, -1,
                                                          self._lstm_output_size + self._action_size + 1], name='lstm_input')  # (1, unroll_step, 256+action_size+1)

            lstm_outputs, lstm_state = tf.nn.dynamic_rnn(self.lstm_cell, lstm_input_reshaped,
                                                         initial_state=initial_state_input, sequence_length=step_size,
                                                         time_major=False, scope=scope)
            lstm_outputs = tf.reshape(lstm_outputs, [-1,
                                                     self._lstm_output_size], name='lstm_output')  # (1,unroll_step,256) for back prop, (1,1,256) for forward prop.
    ...
    def _base_policy_layer(self, lstm_outputs, reuse=False):
        with tf.variable_scope("base_policy{0}".format(self._id), reuse=reuse) as scope:
            # Policy (output)
            W_fc_p, b_fc_p = self._fc_variable([self._lstm_output_size, self._action_size], "base_fc_p")
            beforeSoftmax = tf.add(tf.matmul(lstm_outputs, W_fc_p), b_fc_p, name='policy_before_softmax')
            return tf.nn.softmax(beforeSoftmax)
    ...
    def _base_value_layer(self, lstm_outputs, reuse=False):
        with tf.variable_scope("base_value{0}".format(self._id), reuse=reuse) as scope:
            # Value (output)
            W_fc_v, b_fc_v = self._fc_variable([self._lstm_output_size, 1], "base_fc_v")
            beforeSoftmax = tf.add(tf.matmul(lstm_outputs, W_fc_v), b_fc_v, name='value_before_softmax')
            return tf.reshape(beforeSoftmax, [-1])  # flatten output
    ...
    def _base_loss(self):
        ...
        policy_loss = -tf.reduce_sum(tf.reduce_sum(tf.multiply(log_pi, self.base_a),
                                                   reduction_indices=1) * self.base_adv + entropy * self._entropy_beta)
    ...
    def reset_state(self):
        self.base_lstm_state_out = tf.contrib.rnn.LSTMStateTuple(np.zeros([1, self._lstm_output_size]),
                                                                 np.zeros([1, self._lstm_output_size]))

    def run_policy_and_value(self, sess, s_t, last_action_reward):
        # This run_policy_and_value() is used when forward propagating.
        # so the step size is 1.
        pi_out, v_out, self.base_lstm_state_out = sess.run([self.base_pi, self.base_value, self.base_lstm_state],
    ...
    def run_value(self, sess, s_t, last_action_reward):
        ...
        v_out, _ = sess.run([self.base_value, self.base_lstm_state],
