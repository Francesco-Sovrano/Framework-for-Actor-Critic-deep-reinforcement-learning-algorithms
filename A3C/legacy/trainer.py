...
class Trainer(object):
    def __init__(self, thread_index, global_network, initial_learning_rate, learning_rate_input, grad_applier, env_type,
                 entropy_beta, local_t_max, gamma, max_global_time_step, device):
        self.stats = {}
        ...
        self.device = device
        # build network
        self.local_network = MultiAgentModel(self.thread_index, state_shape, agents_count, self.action_size,
        ...
        for i in range(self.local_network.agent_count):
            ...
            self.apply_gradients.append(
                self.grad_applier.minimize_local(local_agent.total_loss, global_agent.get_vars(),
    ...
    def _print_log(self, global_t):
        if (self.thread_index == 0) and (self.local_t - self.prev_local_t >= PERFORMANCE_LOG_INTERVAL):
            ...
            print("### Performance : {} STEPS in {:.0f} sec. {:.0f} STEPS/sec. {:.2f}M STEPS/hour".format(global_t,
                                                                                                          elapsed_time,
                                                                                                          steps_per_sec,
